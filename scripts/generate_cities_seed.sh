...
with output_path.open("w", encoding="utf-8") as fh:
    ...
    fh.write("BEGIN;\n")
    fh.write("TRUNCATE TABLE public.cities RESTART IDENTITY;\n")
    fh.write("TRUNCATE TABLE public.countries;\n\n")
    ...
    for idx, record in enumerate(sorted(city_rows, key=lambda c: c["slug"])):
        formatted_values = ", ".join(
